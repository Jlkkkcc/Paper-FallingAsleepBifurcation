function [ft, ft_dscrp] = sleep_ft_extract_new(y, Fs, python_directory, iflinux, varargin)
...
% 09/05/2023, Anastasia: added option to define the range of band-pass
% filtering when calling the function. Added gamma band. Added entropy rate extraction (through Lempel-Ziv and CTW) 

% 22/05/2023: Anastasia: added extraction of aperiodic spectral component
% (exponent spectral slope)

% 24/05/2023: Anastasia: added extraction of microstructural sleep events
% (k complexes, sleep spindles, monophasic and biphasic vertices)

%% Start

ifsavefeaturenames = 0;
ifgamma = 0;
...
% python directory for Mac
% python_directory = '/Library/Frameworks/Python.framework/Versions/3.10/bin/python3'

% Input the limits of the bandpass filter
if isempty(varargin)
    min_freq = 0.1;
    max_freq = 32;
else
    for i = 1:length(varargin)
        switch varargin{i}
            case 'Min Freq'
                min_freq = varargin{i+1};
            case 'Max Freq' 
                max_freq = varargin{i+1};
            case 'Save Feature Names'
                if varargin{i+1} == 1
                    ifsavefeaturenames = 1;
                end
        end
    end
end
...
% MA (moving average) filter for removing 1/f component
if_aperiodic = 0; 
...
if ifproc     % Normal pre-processing
    % filter EEG, broad BP
    order = 2;
    lowFreq0 = min_freq;    % 0.1
    highFreq0 = max_freq;    % 32
...
psd_y_norm = psd_y ./ sum(psd_y);    % Normalize to total power -> use for entropy features
...
if ifgamma
    ft.AGratio = log(alpha_power/gamma_power);
    ft.BGratio = log(beta_power/gamma_power);
    ft.TGratio = log(theta_power/gamma_power);
    ft.DGratio = log(delta_power/gamma_power);
end
...
if ifgamma
    ft_dscrp = [ft_dscrp,{'delta','theta','alpha','beta','gamma','DTratio','DAratio','DBratio','TAratio','TBratio','ABratio','GAration', 'GBratio', 'GTratio','GDratio','Transratio'}];
else
...
if ifgamma
    f_gamma = f(gamma_idx); 
    ft.peakgamma = f_gamma(find(psd_y_norm(gamma_idx) == max(psd_y_norm(gamma_idx)),1));
end

if ifgamma
    ft_dscrp = [ft_dscrp,{'deltapeak','thetapeak','alphapeak','betapeak', 'gammapeak'}];
else
...
for i = 1:num_epc_unit-1
    for ii = i+1:num_epc_unit
        ...
        data1 = y_epc{i};
        data2 = y_epc{ii};
        [Cxy_y(cnt,:),f_cohere] = mscohere(data1,data2,[],[],0:min_freq:max_freq,Fs);
    end
end
...
if ifgamma
    gamma_idx = (f_cohere>gamma_bd(1) & f_cohere <gamma_bd(2)); 
end 
...
if ifgamma
   ft.TCgamma = mean(Cxy_y_avg(gamma_idx), 'omitnan');
end 


if ifgamma
    ft_dscrp = [ft_dscrp,{'TCdelta','TCtheta','TCalpha','TCbeta', 'TCgamma'}];
else
...
if ifgamma
    ...
    y_filt_gamma = filtfilt(d_gamma, y); 
end 
...
if ifgamma
    y_gamma_analytic = hilbert (y_filt_gamma);
end 
...
if ifgamma
    amp_gamma = abs(y_gamma_analytic);
end 
...
if ifgamma
    zt_DG = amp_gamma(1:idx_cycle_delta).*exp(j*phase_delta(1:idx_cycle_delta));
    zt_TG = amp_gamma(1:idx_cycle_theta).*exp(j*phase_theta(1:idx_cycle_theta));
    zt_AG = amp_gamma(1:idx_cycle_alpha).*exp(j*phase_alpha(1:idx_cycle_alpha));
    zt_BG = amp_gamma(1:idx_cycle_beta).*exp(j*phase_beta(1:idx_cycle_beta));
else
    zt_DG = NaN;
    zt_TG = NaN;
    zt_AG = NaN;
...
if ifgamma
    ft.DGcouple = abs(sum(zt_DG)/sum(amp_gamma(1:idx_cycle_delta)));
    ft.TGcouple = abs(sum(zt_TG)/sum(amp_gamma(1:idx_cycle_theta)));
    ft.AGcouple = abs(sum(zt_AG)/sum(amp_gamma(1:idx_cycle_alpha)));
    ft.BGcouple = abs(sum(zt_BG)/sum(amp_gamma(1:idx_cycle_beta)));
    
end 
    

if ifgamma
    ft_dscrp = [ft_dscrp,{'DTcouple','DAcouple','DBcouple','TAcouple','TBcouple','ABcouple', 'DGcouple', 'TGcouple', 'AGcouple', 'BGcouple'}];
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Complexity features (entropy rate- and Lempel-Ziv complexity-related
% functions from https://github.com/pmediano/EntRate) 

ifcomplexity = 1;

if size(y,1)>size(y,2)     %make sure it's a row vector
    y = transpose(y);
end
...
if ifcomplexity
    ...
    entropy_rate_lz = lempel_ziv*log2(length(binarized_y))/length(binarized_y); % haven't run yet 
    
    %ft.entropy_rate = entropy_rate;
    %ft.lempel_ziv = lempel_ziv;
    ft.entropy_rate_lz = entropy_rate_lz; % haven't run yet 
    %ft_dscrp = [ft_dscrp, 'Entropy Rate CTW', 'Entropy Rate LZ'];
    ft_dscrp = [ft_dscrp, 'Entropy Rate LZ'];
end 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Micromeasures of sleep and alertness (from
% https://github.com/SridharJagannathan/microMeasAlertness_HumanEEG )
%[monophasic_counts, biphasic_counts, spindle_counts, kcomp_counts] = extract_microstruct_events(y_proc, Fs); 

%ft.monophasic_counts = monophasic_counts;
%ft.biphasic_counts = biphasic_counts;
%ft.spindle_counts = spindle_counts;
%ft.kcomp_counts = kcomp_counts;

%ft_dscrp = [ft_dscrp, 'Number of Monophasic Vertices', 'Number of Biphasic Vertices', 'Number of Sleep Spindles', 'Number of K-complexes']; 



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if ifsavefeaturenames
    ft_names = ft_dscrp;
    save("feature_extraction/expanded_feature_base.mat", 'ft_names', '-mat')
end 
