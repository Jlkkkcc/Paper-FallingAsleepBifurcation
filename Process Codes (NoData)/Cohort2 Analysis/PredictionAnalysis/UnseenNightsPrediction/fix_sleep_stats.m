...
cd '/Users/anastasia/Dropbox/ISN work/Work on new dataset/Critical Point Prediction Corrected/'
addpath '/Users/anastasia/Dropbox/ISN work/Work on new dataset/Critical Point Prediction Corrected'
addpath '/Users/anastasia/Dropbox/ISN work/Work on new dataset/Mean S-variable for prediction/Functions'
addpath '/Users/anastasia/Dropbox/ISN work/Work on new dataset/Critical Point Prediction'
...
load WithTestNightsNumbersSummaryTable.mat
...
for row_idx = 1:height(uniqueTable)
    ...
    if isnan(uncut_xx_smoothed(1))
        plot_bifurcation(dd_orig(firstNonNaNIndex:end, :), uncut_xx_smoothed(firstNonNaNIndex:end), uncut_tvec(firstNonNaNIndex:end), saveDirParticipant, uniqueTable.Patient_ID{row_idx}, init_params, 1, 0)
    else
        plot_bifurcation(dd_orig, uncut_xx_smoothed, uncut_tvec, saveDirParticipant, uniqueTable.Patient_ID{row_idx}, init_params, 1, 0)
    end 
    ...
    [criticalS, c3sol] = plot_bifurcation(dd, xx_smoothed, tvec, saveDirParticipant, uniqueTable.Patient_ID{row_idx}, params_tuned, 0, 1);
    ...
    [realCrossingValues_model, realCrossingTimePoints_model, crossingValues_model, crossingTimePoints_model, crossingDetails_model] = identify_crossing_points(xx_smoothed, criticalSValue, tvec, saveDirParticipant, 1, 'Bifurcation', dd(:, 1));
    ...
    for n_n = 1:length(estimatedSVariables)
        ...
        [realCrossingValues, realCrossingTimePoints, crossingValues, crossingTimePoints, crossingDetails] = identify_crossing_points(xx_smoothed, criticalSValue, tvec_night, saveDirParticipant, 0, 'NightNumber', n_n);
...
matFilename = fullfile(saveDirAdjustedBifurcation, 'uniqueTable.mat');
