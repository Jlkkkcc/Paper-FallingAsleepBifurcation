...
ft_touse = [15,46,47,53,2,9,33,41,54];  % FPC1 representative features
...
for nft = 1:length(ft_touse)
    ...
    clear ts_sbj_nomed distmat_allsbj_nomed
    distmat_allsbj_nomed = NaN(num_sbjs,max_len);
    ftid_now = ft_touse(nft);
    for sbj = 1:num_sbjs
        if ifelig(sbj)
            ns = ns+1;
            isart_sbj_now = isart_allsbj{sbj};
            ...
            ftmatonset = NaN(lengthonset,length(ftid_now));
            for ne = 1:lengthonset

                if isart_sbj_now(onsetidx(ne))
                    ftmatonset(ne,:) = NaN(1,length(ftid_now));
                    continue
                end

                ftvec_allch = ft_sbj_patch{sbj}.ck{onsetidx(ne)}.ft_ch(:,ftid_now);
                ftvec_allch(ftvec_allch(:,1)==0,:) = NaN;
                ftmatonset(ne,:) = nanmean(ftvec_allch,1);
                % Global z-score normalisation
                ftmatonset(ne,:) = (ftmatonset(ne,:)-mean_allft(ftid_now))./std_allft(ftid_now);        % Global normlisation
            ...
            ftmat = NaN(num_epcs_sbj,length(ftid_now));

            for ne = 1:num_epcs_sbj

                ftvec_allch = ft_sbj_patch{sbj}.ck{ck_idx(ne)}.ft_ch(:,ftid_now);
                ftvec_allch(ftvec_allch(:,1)==0,:) = NaN;
                ftmat(ne,:) = mean(ftvec_allch,1,'omitnan');
                % Global z-score normalisation
                ftmat(ne,:) = (ftmat(ne,:)-mean_allft(ftid_now))./std_allft(ftid_now);        % Global normlisation
    ...
    %%%%%%%% Saving
    distmat_perft{nft} = distmat_allsbj_nomed;
    ts_perft_allsbj{nft} = ts_sbj_nomed;
    %
end
...
clear ft_sbj ft_sbj_patch
save('TS_SleepDistIndividuals_indivft_ftadd.mat')
